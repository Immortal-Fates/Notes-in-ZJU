...
- **构造正负样本**：同一实例的不同增强作为正样本，不同实例作为负样本。
- **使用对比损失**：例如 InfoNCE，让正样本相似度高于负样本。
- **大批量或 memory bank**：提供更多负样本，使表示学习更稳定。
- **跨模态匹配**：把天然配对的样本作为正对，例如图片和对应文本。

Pros:

- 能在无标签或弱标签数据上学习强表征。
- 学到的 embedding 适合检索、聚类、迁移和跨模态对齐。
- 目标直观，和相似度学习天然匹配。

Cons:

- 正负样本构造很关键，错误构造会误导模型。
- 常需要大 batch、memory bank 或复杂采样策略。
- 学到的不变性由数据增强决定，可能丢失任务需要的信息。
