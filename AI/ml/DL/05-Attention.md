...
- Conclustion: Both CNNs and self-attention enjoy **parallel computation** and self-attention has the **shortest maximum path length**. However, the **quadratic computational complexity** with respect to the sequence length makes self-attention prohibitively slow for very long sequences.
  ...
  |                         | RNNs                                          | Transformers                                             |
  | ----------------------- | --------------------------------------------- | -------------------------------------------------------- |
  | Long-Range Dependencies | Good in theory, but hard in practice          | Good in practice, through self-attention over full input |
  | Parallelizability       | No – sequential computation  across timesteps | Yes – process tokens in  parallel                        |
  | Compute & Memory Use    | O(N),O(N)                                     | O(N^2),O(N)                                              |
  | Inductive Bias          | Strong – inherent temporal  structure         | Weak – needs to learn from  data                         |
