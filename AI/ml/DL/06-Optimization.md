# Optimization Algorithms

神经网络与梯度下降

- [ ] 掌握神经网络优化算法：SGD、Adam等

<!--more-->

[TOC]

## 神经网络的基本原理

凸优化问题有助于分析算法的特点。 毕竟对大多数非凸问题来说，获得有意义的理论保证很难，但是直觉和洞察往往会延续。所以我们研究一个优化算法，常常将其运用于如下问题：
$$
\min f(x)=\frac{1}{2}x^TQx+c^Tx+b
$$

### 大规模优化挑战

在统计学和机器学习中，许多问题的核心可归结为以下优化问题：
$$
\min_x f(x) = \frac{1}{m} \sum_{i=1}^m f_i(x)
$$
其中$m$表示样本量（即子函数个数）

- 计算挑战：当$m$极大时，全梯度计算需要巨大计算量，甚至在$m$为无穷时无法实现。

- 解决策略：用随机子函数的梯度来估计目标函数的全梯度，这类方法称为随机方法。在实际应用中，这类方法通常比确定性的梯度下降方法更快

## 梯度下降（GD）及其变种

### GD(Gradient Descent)

**定义**：沿着负梯度方向更新参数，使函数值逐步减小。
$$
\theta_{t+1} = \theta_t - \eta \nabla f(\theta_t)
$$

- 我们使用梯度下降进行参数更新策略，用于**最小化损失函数**

- 反向传播（Backpropagation）是**计算梯度的方法**，用于**高效计算损失函数对模型所有参数的偏导数**。本质是链式法则（chain rule）的系统化应用。

### 批量 / 随机 / 小批量梯度下降

统一为：

$$
\theta_{t+1} = \theta_t - \eta \frac{1}{|B_t|}\sum_{i\in B_t}\nabla f_i(\theta_t)
$$

其中 $B_t$ 是当前批次。

| 方法       | 批大小 $|B_t|$ | 特点                   |
| ---------- | -------------- | ---------------------- |
| Batch GD   | n (全量)       | 梯度精确但慢           |
| SGD        | 1              | 方差大但快、可在线更新 |
| Mini-batch | 小于 n         | 综合两者优点           |

#### 随机梯度下降SGD

SGD（stochastic gradient descent）

- 核心思想：每次迭代中随机选择一个样本i，然后利用该样本的梯度来更新参数

  为什么SGD能有效果？我们强调随机梯度$\nabla f_i(x)$是对完整梯度$\nabla f(x)$的无偏估计：
  $$
  E_i\nabla f_i(x)=\frac{1}{n}\sum \nabla f_i(x) = \nabla f(x)
  $$

- 算法步骤

  1. 在每次迭代中随机选择一个样本 $i \sim \text{Uniform}(1,m)$

  2. 计算单个样本的梯度$\nabla f_i(x_t)$

  3. 参数更新$x_{t+1} = x_t - \eta \nabla f_i(x_t)$，其中 $\eta$ 为学习率 (learning rate)

- 特性分析

  | 优点             | 缺点               |
  | ---------------- | ------------------ |
  | 单次迭代计算量低 | 收敛路径震荡       |
  | 能逃离局部极小点 | 学习率需精细调节   |
  | 适合大规模数据集 | 不保证严格单调收敛 |

#### 小批量梯度下降Mini-batch Gradient Descent

- 核心思想：每次迭代中随机选择一个小批量样本，然后利用这些样本的梯度平均值来更新参数——**计算效率**

- 特性分析：小批量梯度下降在SGD和批量梯度下降之间取得平衡，既减少了计算量，又降低了更新的波动性。

  好的副作用：使用平均梯度减小了方差

### 动量算法（Momentum）

- 核心思想：在迭代过程中引入动量项，利用历史梯度信息来加速收敛并减少震荡
  $$
  v_t = \beta v_{t-1}+g_{t,t-1},~\beta \in(0,1)
  $$

> Tips：参数像一个有质量的小球，梯度是坡度的“重力”。小球累积动量，使其在平坦方向加速，在陡峭方向平滑振荡。

### Nesterov加速梯度（Nesterov Accelerated Gradient, NAG）

- 核心思想：改进的动量优化算法，其核心在于**前瞻性梯度计算**。NAG 在更新参数时先沿当前动量方向进行一步预测，然后在预测点计算梯度，从而更准确地调整参数方向

- 算法步骤：

  1. 生成辅助变量$v_k$：$v_k=x_k+\gamma(x_k-x_{k-1})$

     其中，$\gamma$是动量参数，常取0.9

  2. 梯度下降：
     $$
     x_{k+1}=v_k-\alpha_k \nabla f(v_k)
     $$
     其中，$\alpha_k$是步长

- 优缺点：

  | **优点**                                                     | **缺点**                                               |
  | ------------------------------------------------------------ | ------------------------------------------------------ |
  | **更快的收敛速度**：通过前瞻性梯度修正方向，减少震荡，加速收敛至局部最优（理论收敛速率 O(1/t2)，优于标准梯度下降的 O(1/t)） | **超参数敏感**：需仔细调节学习率 η 和动量系数 γ        |
  | **高曲率适应性强**：在病态条件或高曲率区域表现优于传统动量法 | **非凸问题局限**：对非凸目标函数可能陷入鞍点或局部极小 |
  | **减少震荡**：前瞻梯度计算使更新方向更贴近实际下降路径       | **计算复杂度略高**：需额外计算预测点梯度               |

  特别在处理大规模数据集和高维参数空间时表现出色。

### 自适应学习率方法

#### Adagrad（Adaptive Gradient）

Adagrad是一种随机梯度下降的方法

- 引言：稀疏特征，鉴于学习率下降，我们可能最终会面临这样的情况：常见特征的参数相当迅速地收敛到最佳值，而对于不常见的特征，我们仍缺乏足够的观测以确定其最佳值。 换句话说，学习率要么对于常见特征而言降低太慢，要么对于不常见特征而言降低太快。

  解决此问题的一个方法就是记录我们看到特定特征的次数，然后将其用作调整学习率：$\eta_i = \frac{\eta_0}{\sqrt{s(i,t)+c}},~s(i,t)$这里计下了我们截至$t$观察到功能$i$的次数

- 核心思想：为每个参数动态调整学习率，是一种**自适应学习率优化算法**

  通过将上面粗略的计数器$s(i,t)$替换为先前观察所得梯度的平方之和来解决这个问题
  $$
  s(i,t+1)=s(i,t)+(\partial_if(x))^2
  $$

  - **高频参数**（梯度大且频繁更新的参数）降低学习率，避免震荡；
  - **低频参数**（梯度小或稀疏的参数）增大学习率，加速收敛。 通过累积历史梯度平方和，Adagrad 自动适应不同参数的特征，特别适合稀疏数据（如自然语言处理任务）

  AdaGrad算法会在单个坐标层面动态降低学习率

- 算法步骤：

  1. 初始化：

     - 全局学习率$\eta$
     - 梯度平方累计变量$r=0$
     - 小常数$\epsilon$

  2. 迭代更新

     1. 计算当前梯度

     2. 累积梯度平方和：$r_t=r_{t-1}+g_t\odot g_t$

        > $\odot$哈达玛积：两个同维矩阵的元素对应相乘

     3. 调整学习率；$\eta_t = \frac{\eta}{\sqrt{r_t+\epsilon}}$

     4. 更新参数

- 优缺点

  | **优点**                                                     | **缺点**                                                     |
  | ------------------------------------------------------------ | ------------------------------------------------------------ |
  | **自适应学习率**：无需手动调节，适合高维稀疏数据（如 NLP 中的词向量）。 | **学习率衰减过快**：累积梯度平方和持续增大，导致后期学习率趋近于零，模型停止更新。 |
  | **梯度方向优化**：对低频参数更敏感，加速稀疏特征的学习。     | **内存消耗高**：需为每个参数存储梯度平方累积量，参数量大时内存占用显著。 |
  | **数值稳定性**：引入 ϵ 避免分母为零，适合非凸优化问题。      | **依赖初始学习率**：初始 η 设置不当可能导致早期收敛慢或震荡。 |

#### RMSprop（Root Mean Square Propagation）

Adagrad 算法的改进版本，旨在解决 Adagrad 因累积全部历史梯度导致学习率**过早衰减**的问题，Adagrad:$s(i,t+1)=s(i,t)+(\partial_if(x))^2$由于缺乏规范化，没有约束力，$s_t$持续增长，几乎上是在算法收敛时呈线性递增。

- 核心思想：$s(i,t+1)=\gamma s(i,t)+(1-\gamma)(\partial_if(x))^2$
  - **指数加权平均**：通过衰减系数（如 0.9）动态调整历史梯度的影响，仅保留近期梯度信息，避免长期累积导致学习率过小。
  - **自适应学习率**：每个参数的学习率根据其梯度幅度的均方根（RMS）动态调整，梯度大的参数降低学习率，梯度小的参数增大学习率

#### Adadelta

Adadelta是AdaGrad的另一种变体

- 核心思想：减少了学习率适应坐标的数量。 此外，广义上Adadelta被称为没有学习率，因为它使用变化量本身作为未来变化的校准

- 算法步骤：

  1. 泄漏梯度平方更新

     维护历史梯度平方的指数加权平均（ρ为衰减因子）：
     $$
     s_t = \rho s_{t-1} + (1 - \rho) g_t'^2
     $$

  2. 参数更新规则

     使用调整后的梯度进行参数更新：
     $$
     x_t = x_{t-1} - g_t'
     $$

  3. 梯度调整计算

     通过历史参数更新量缩放原始梯度（ϵ为数值稳定项）：
     $$
     g_t' = \frac{\sqrt{\Delta x_{t-1} + \epsilon}}{\sqrt{s_t + \epsilon}} \odot g_t
     $$

  4. 参数更新量更新

     维护参数更新量的指数加权平均：
     $$
     \Delta x_t = \rho \Delta x_{t-1} + (1 - \rho) g_t'^2
     $$

### Adam and AdamW

> Adam（Adaptive Moment Estimation）

一种自适应学习率优化算法，**融合了动量法（Momentum）和RMSProp的优点**。将很多优化技术汇总到了这一个方法里面，但是也有一些问题改进

- 核心思想：

  - **一阶矩估计（动量）**：跟踪梯度的指数加权平均，保持参数更新方向的稳定性。
  - **二阶矩估计（自适应学习率）**：计算梯度平方的指数加权平均，根据梯度幅值调整学习率。
  - **偏差校正**：修正初始阶段因零初始化导致的矩估计偏差，确保更新量的准确性。

- 算法步骤：

  - 初始化：

    - 初始参数：$ \theta_0 $
    - 学习率：$ \alpha = 0.001 $（默认）
    - 一阶矩衰减率：$ \beta_1 = 0.9 $
    - 二阶矩衰减率：$ \beta_2 = 0.999 $
    - 数值稳定常数：$ \epsilon = 10^{-8} $
    - 初始一阶矩 $ m_0 = 0 $，二阶矩 $ v_0 = 0 $

  - 迭代过程（时间步 $ t=1,2,\dots $）：

    - 计算当前梯度：

    $$
    g_t = \nabla_\theta L(\theta_{t-1})
    $$

    - 更新一阶矩（动量）：

    $$
    m_t = \beta_1 \cdot m_{t-1} + (1 - \beta_1) \cdot g_t
    $$

    - 更新二阶矩（自适应学习率）：

    $$
    v_t = \beta_2 \cdot v_{t-1} + (1 - \beta_2) \cdot g_t^2
    $$

    - 偏差校正（消除初始零偏置）：

    $$
    \hat{m}_t = \frac{m_t}{1 - \beta_1^t}, \quad \hat{v}_t = \frac{v_t}{1 - \beta_2^t}
    $$

    - 更新参数：

    $$
      \theta_t = \theta_{t-1} - \frac{\alpha}{\sqrt{\hat{v}_t} + \epsilon} \cdot \hat{m}_t
    $$

- 优缺点

  | **优势**         | **说明**                                                     |
  | ---------------- | ------------------------------------------------------------ |
  | **自适应学习率** | 每个参数独立调整学习率，梯度大的方向步长减小，梯度小的方向步长增大。 |
  | **高效收敛**     | 结合动量与自适应机制，在非凸优化问题中收敛速度通常快于SGD、RMSProp。 |
  | **处理稀疏梯度** | 对低频参数（如NLP中的词向量）自动增大更新步长，提升训练效率。 |
  | **抗噪声能力强** | 通过指数加权平均平滑梯度噪声，适应高噪声数据场景。           |

  | **局限性**           | **说明**                                                     |
  | -------------------- | ------------------------------------------------------------ |
  | **超参数敏感**       | β1,β2,α 需调优，不当设置可能导致收敛不稳定。                 |
  | **内存占用较高**     | 需存储一阶和二阶矩变量，参数量较大时内存消耗显著。           |
  | **局部最优风险**     | 在部分非凸问题中可能陷入鞍点，需结合预热（Warmup）或学习率衰减。 |
  | **长期训练性能下降** | 因自适应学习率随训练衰减，后期可能不如带动量的SGD            |

**AdamW 改进**：将权重衰减项从梯度中分离，解决 Adam 在正则化中的失效问题。

> [!TIP]
>
> Adam 通常在早期收敛更快，但 SGD + Momentum 在后期泛化更好。

### Proximal Gradient Descent

近端梯度下降Proximal Gradient Descent

许多优化问题具有如下的复合结构：

\[
\min_x F(x) = f(x) + g(x)
\]

其中：

- \(f(x)\)：可微、光滑、梯度 Lipschitz 连续  
- \(g(x)\)：可能不可微（如 L1 正则）、可能是复杂结构的正则项  

常见场景包括：

- 稀疏建模（Lasso）
- 稀疏神经网络、模型压缩
- 图像去噪、变分模型（Total Variation）
- 核范数最小化、矩阵补全
- 各类带投影或约束的优化问题

传统梯度下降无法处理不可微项 \(g(x)\)，PGD 通过引入 **近端算子（proximal operator）** 解决这一问题。

在介绍近端算子之前，我们先介绍次梯度(sub gradient)，是梯度的推广，主要用于不可微函数

**定义 (次梯度)：**g是凸函数$f(x)$在$x$点的次梯度当且仅当：$\forall y\in C$满足：
$$
\begin{equation} f(y)\geq f(x) + g^\top (y-x) \end{equation}
$$
注意：这里的函数必须是**凸函数**，而且，凸函数还有一个重要的性质：

如果 $f(x)$ 是**可微凸函数**，那么对于任何 $x, y\in C$ ：
$$
\begin{equation} f(y)\geq f(x) + \nabla f(x)^\top (y-x) \end{equation}
$$
凸函数上面的任意一点，**过该点沿梯度** \nabla f(x) **方向的直线总是处在函数的下方**。

再对比一下次梯度的定义，有以下几个**重要的结论**：

- $f$ 必须凸函数。
- 次梯度**不唯一**，它可以是**一个集合**。
- 如果 $f(x)$ 在 $x$ 点处可导，那么这个点的次梯度不再是一个集合，而变成了唯一的一个值，这个值是 $\nabla f(x)$ 。

**定义 (次微分)：**函数 f(x) 在 x 处的次微分定义为函数 f(x) 在 x 处的次梯度的集合。
$$
\begin{equation}\partial f(x) = \{g: g \text{是}f\text{在}x\text{的次梯度}\}\end{equation}
$$
注意到 $\partial f(x)$ 是一个凸集。我们证明一下：

设集合 $C=\partial f(x)$ ，任取 $m,n\in C$ ，则对于函数 f(x) 在点 x 有：
$$
\begin{equation} f(y)\geq f(x) + m^\top (y-x)\end{equation} \tag{p.1}
$$

$$
\begin{equation} f(y)\geq f(x) + n^\top (y-x)\end{equation}\tag{p.2}
$$

做变换： $\theta\cdot (p.1)+(1-\theta)\cdot (p.2)$ 有：
$$
\begin{equation} f(y)\geq f(x) + (\theta m+(1-\theta)n)^\top (y-x)\end{equation}\tag{p.3}
$$
故 $\theta m+(1-\theta)n\in C$。得证。

**次梯度性质（最优性条件）**：$f(x^*) = \min f(x) \Leftrightarrow 0\in \partial f(x^*)$

接下来我们来看Proximal Operator（近端算子），其是从次梯度零点定义出来的

给定函数 \(g(x)\)，其近端算子定义为：

\[
\text{prox}_{\lambda g}(v)
= \arg\min_x \left(
g(x) + \frac{1}{2\lambda} \|x - v\|^2
\right)
\]

相当于求解以下问题的最优解（带入次梯度最优性条件）：
$$
\min_x \phi(x) = g(x)+\frac{1}{2\lambda} \|x - v\|^2
$$
核心直觉

- 近端算子在点 \(v\) 附近寻找一个折中的点，使其既接近 \(v\) 又能降低 \(g(x)\)。
- 对非光滑正则项进行“修正”。

近端梯度下降（PGD）算法：在点 $v$ 附近找到一个能兼顾 g(x) 的最优点

目标：

\[
\min_x f(x) + g(x)
\]

PGD 更新公式：

\[
x_{k+1}
= \text{prox}_{\alpha g}\left(
x_k - \alpha \nabla f(x_k)
\right)
\]

1. **梯度下降（处理可微部分 \(f\)）**  
   \[
   y_k = x_k - \alpha \nabla f(x_k)
   \]

2. **近端映射（处理不可微部分 \(g\)）**  
   \[
   x_{k+1} = \text{prox}_{\alpha g}(y_k)
   \]

## CheatSheet

| 方法       | 关键思想        | 优点       | 缺点       | D2L 推荐场景       |
| ---------- | --------------- | ---------- | ---------- | ------------------ |
| GD         | 全梯度下降      | 理论稳     | 慢         | 小数据集理论分析   |
| SGD        | 单样本梯度      | 简单高效   | 抖动大     | 在线学习           |
| Mini-batch | 平均小批梯度    | 稳定高效   | 超参敏感   | 深度学习默认       |
| Momentum   | 累积梯度动量    | 加速平滑   | 参数需调   | 高曲率地形         |
| NAG        | 前瞻校正        | 减震效果好 | 复杂       | 高维任务           |
| AdaGrad    | 坐标自适应      | 稀疏特征好 | 学习率早衰 | 词向量训练         |
| RMSProp    | 滑动平均        | 稳定快速   | 参数多     | RNN训练            |
| AdaDelta   | 去除全局 η      | 鲁棒       | 慢         | 调参困难时         |
| Adam       | 一阶+二阶矩估计 | 快速收敛   | 泛化差     | 初期实验           |
| AdamW      | 解耦权重衰减    | 更稳       | 略慢       | Transformer 类模型 |

## Tricks

- **学习率调度（Scheduler）**：Step、Cosine、Warmup、OneCycle。
- **梯度裁剪（Gradient Clipping）**：限制梯度范数以防爆炸。
- **混合精度训练（FP16）**：提升速度、减少显存。
- **权重衰减（L2 正则化）**：防止过拟合。

## References

- [Tutorial — Ceres Solver (ceres-solver.org)](http://ceres-solver.org/tutorial.html)
- [An Introduction to Optimization: With Applications to Machine Learning - Edwin K. P. Chong, Wu-Sheng Lu, Stanislaw H. Zak - Google Books](https://books.google.co.jp/books?hl=en&lr=&id=uEDUEAAAQBAJ&oi=fnd&pg=PR15&dq=info:WSaWZthIywYJ:scholar.google.com&ots=qAtUVnxFtM&sig=Sl77RBaLcYWOmv7FMLzQZg-qcGg&redir_esc=y#v=onepage&q&f=false)--ch13
- [d2l](https://d2l.ai/)
- [近端梯度下降](https://zhuanlan.zhihu.com/p/277041051)
