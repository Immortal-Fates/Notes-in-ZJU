...
小目标在 object detection 中为什么容易失败？哪些机制能够提升 recall、representation quality 和 localization stability？

### Paper Matrix

| Paper / Method | 为什么放入这条线 | 在线中的作用 | 详细笔记 |
| --- | --- | --- | --- |
| Dome-DETR | 使用 density-guided feature computation 和 adaptive query allocation。 | Density-aware tiny-object DETR | [Dome-DETR](#small-object) |
| DPNet | 根据每张图动态选择输入/特征分辨率。 | Adaptive resolution route | [DPNet](#small-object) |
| MAFE R-CNN | 结合 small-object sample selection 和 category-aware feature memory。 | Assignment + feature enhancement | [MAFE R-CNN](#small-object) |
| LAF-YOLOv10 | 面向 UAV 场景改造 YOLOv10，引入 P2 head、feature pyramid 和小目标模块。 | UAV-specific small-object detector | [LAF-YOLOv10](#small-object) |

### Relation

```mermaid
graph TD
  A[Small Object Failure] -->|foreground density varies| B[Dome-DETR]
  A -->|scale mismatch| C[DPNet]
  A -->|positive samples unstable + weak features| D[MAFE R-CNN]
  A -->|UAV dense tiny scenes| E[LAF-YOLOv10]
```
