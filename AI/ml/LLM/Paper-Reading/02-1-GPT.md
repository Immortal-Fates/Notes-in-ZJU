...
- **Improving Language Understanding by Generative Pre-Training**. Alec Radford et.al. **OpenAI**, **2018**, ([Paper](https://cdn.openai.com/research-covers/language-unsupervised/language_understanding_paper.pdf)).
  ...
  - Core Mechanism:
    ...
    这张图其实非常重要：左边是 language-model pre-training，右边是 task-specific fine-tuning，但 backbone 始终是同一个 decoder-only Transformer。这就是 GPT-1 最核心的结构性思想。
  ...
  - Core Mechanism:
    ...
    - Scaled autoregressive language modeling
      ...
      - How:
        ...
        具体落地上，GPT-2 仍然使用 decoder-only Transformer，只是把参数规模、上下文长度和训练数据规模同步推大，用更大的模型直接做同样的 LM training，再在 inference 时看它是否能从 prompt 里自发完成任务。
    ...
    - WebText + byte-level BPE
      ...
      - How:
        ...
        作者从 Reddit 高质量外链页面构建 WebText，并使用 byte-level BPE 对原始字符串编码后再送进 Transformer。这样模型在训练时看到的是更接近真实 web language distribution 的数据流。
...
- **Language Models are Few-Shot Learners**. Tom B. Brown et.al. **NeurIPS**, **2020**, ([Arxiv](https://arxiv.org/abs/2005.14165)) ([OpenAI](https://openai.com/research/language-models-are-few-shot-learners)).
  ...
  - Core Mechanism:
    ...
    - In-context learning instead of parameter updates
      ...
      - How:
        ...
        ![gpt3-superglue-scaling](./assets/02-1-GPT.assets/gpt3-superglue-scaling-page.png)

        这张图是 GPT-3 最该保留的 figure，因为它直接体现了论文主结论：随着模型规模变大，few-shot performance 在 SuperGLUE 等任务上显著上升。
  ...
  - Pipeline:
    ...
    1. 在大规模 web corpus 上训练一个超大 decoder-only Transformer，目标仍然是标准 next-token prediction。
    2. 下游评估时保持参数冻结，不做 task-specific fine-tuning。
    3. 把每个任务改写成文本 prompt，可包含 instruction 和少量 demonstrations。
    4. 将完整 prompt 输入模型，让它继续 autoregressive generation 或 scoring。
    5. 分别在 zero-shot、one-shot、few-shot 设定下比较任务表现，并观察 scaling 带来的行为变化。
