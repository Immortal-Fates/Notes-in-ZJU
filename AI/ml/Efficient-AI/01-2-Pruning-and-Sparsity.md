...
| BatchNorm Scaling Factor ($\gamma$) Pruning | Uses BN scaling factors to estimate channel importance in Conv–BN structures.<br />Commonly used in Conv → BN → ReLU | The BatchNorm layer’s scaling factor $\gamma$ indicates the relative importance of a channel. | Prune channels with the smalle values $\gamma$               |
...
- Analyze the sensitivity of each layer to determine the per-layer pruning ratio(sensitivity scan)
...
- NetAdapt
  ...
  The goal of NetAdapt is to find a per-layer pruning ratio iteratively to meet a global resource constraint (e.g.,latency, energy, ...)
...
> [!TIP]
>
> when testing the model acceleration, we need a warmup to remove “first‑run” overheads so your latency measurements reflect steady‑state performance.(Initialization: setting up contexts, allocating memory, loading data, creating threads, establishing network/TLS connections.)
>
> ```
> @torch.no_grad()
...
- Why: When training neural networks or fine-tuning quantized neural networks, regularization is added to the loss term to
  - penalize non-zero parameters
  - encourage smaller parameterss
- The most common regularization used for pruning is L1/L2 regularization
  - L1-Regularization: $L\prime = L(x;W)+\lambda|W|$
  - L2-Regularization: $L\prime = L(x;W)+\lambda ||W||^2$

## System & Hardware Support for Sparsity

- EIE(Efficient Inference Engine): Weight Sparsity + Activation Sparsity
- NVIDIA Tensor Core: M:N Weight Sparsity
- TorchSparse & PointAcc: ActivationSparsity

## Slimming

- **Learning Efficient Convolutional Networks through Network Slimming**. Zhuang Liu et.al. **arxiv**, **2017**, ([link](https://arxiv.org/abs/1708.06519v1)).

**Network Slimming** (Liu et al., CVPR 2017) is one of the most influential and widely used **channel pruning** techniques for compressing CNNs.
...
- Motivation: BatchNorm (BN) transforms an input channel $x$ as:
  ...
  > [!NOTE]
  >
  > BN have the two learnable parameters $\gamma$ and $\beta$. Thus, BN $\gamma$ naturally serves as a **channel importance indicator**.

- Core Mechanism:  L1 Regularization on $\gamma$. Slimming modifies the training objective by adding an L1 penalty on all BN γ parameters:
  $$
  L = L_{task}+\lambda \sum |\gamma_i|
  $$
