...
Docker 有一些重要的限制需要注意。首先，容器镜像通常是平台特定的——为 `Linux/amd64` 构建的镜像在没有模拟的情况下无法在 `linux/arm64`（apple silcon Mac）上原生运行，这很慢。其次，Docker 容器需要 Linux 内核，所以在 macOS 和 Windows 上，Docker 实际上在底层运行一个轻量级的 Linux 虚拟机，增加了开销。第三，Docker 的隔离性比虚拟机弱——容器共享主机内核，这在多租户环境中是安全隐患。
