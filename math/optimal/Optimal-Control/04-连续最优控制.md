...
![image-20250414214107932](assets/04-连续最优控制.assets/image-20250414214107932.png)
...
![lQDPJwMj13pGsPfNA8DNBQCwjqqIMQElyiEH3PS4sZbuAA_1280_960](assets/04-连续最优控制.assets/lQDPJwMj13pGsPfNA8DNBQCwjqqIMQElyiEH3PS4sZbuAA_1280_960.jpg)
...
![image-20250415110909013](assets/04-连续最优控制.assets/image-20250415110909013.png)
...
> [!NOTE]
>
> 这里$p(t)$为协态变量，对应之前的$\lambda$，是把动力学约束并入优化时引入的拉格朗日乘子。它满足协态方程$\dot λ = -∂H/∂x$，并和终端代价有关。物理上可理解为“对状态变化的边际代价/影子价格
...
- 优化原理：极小值原理替代导数为零条件
- 在上述哈密顿框架基础上，再加上最小化条件u*(t) = argmin_u H(x, u, λ, t)（含控制约束），并给出更完整的必要条件（含切换、互补、终端横截条件等）。它是**最优控制的核心必要性定理**

> [!IMPORTANT]
>
> 把“带控制约束”的最优控制问题转化为一组**状态-协态的两点边值问题**，可直接用于工程求解与数值计算
...
定义哈密顿函数：
$$
\mathcal{H}(x, u, p, t) = g(x, u, t) + p^T f(x, u, t)
$$
其中 $p(t)$ 为协态变量（拉格朗日乘子）。

则性能指标取得局部极小值的必要条件是最优控制 $u(t)$ 和最优控制下的状态变量 $x(t)$ 满足如下条件：

1. 极值条件：最优控制 $u(t) \in U$，对任意容许的控制变量 $u'(t) \in U$，在几乎任意时刻 $t \in [t_0, t_f]$，有
$$
...
$$
  ...
  等价地，可写为
$$
  u^*(t) \in \arg\min_{u \in U} \mathcal{H}(x(t), u, p(t), t)
$$
  当 $U=\mathbb{R}^m$（无控制约束）且 $\mathcal{H}$ 对 $u$ 可微时，退化为
$$
  \frac{\partial \mathcal{H}}{\partial u}(x(t), u(t), p(t), t) = 0
$$
...
定义切换函数：
$$
\phi(t) = \frac{\partial \mathcal{H}}{\partial u}(x(t), u(t), p(t), t)
$$
当 $u$ 受到区间约束 $u \in [u_{\min}, u_{\max}]$ 时，最优控制常呈现饱和/开关结构：

- 若 $\phi(t) > 0$，取 $u^*(t) = u_{\min}$
- 若 $\phi(t) < 0$，取 $u^*(t) = u_{\max}$
- 若 $\phi(t) = 0$，进入奇异弧（需更高阶条件）

当终端时刻自由且 $h$ 不显含 $t$ 时，常见结论为
$$
\mathcal{H}(t_f) = 0
$$
...
> [!IMPORTANT]
>
> 为什么LQR 的最优控制一定是线性状态反馈：
>
> 在 **LQR = 线性系统 + 二次型代价** 这个组合下，最优控制问题具有一种很强的“闭合性”：**最优代价函数必然是二次型**，而最优控制由对代价函数的梯度给出，于是自然变成 **线性状态反馈**
...
下面我们求解这几个必要条件，推导出Riccati方程和最优控制律
...
> 这里为什么假设协态与状态成线性关系：
>
> - 由终端条件 $\lambda(t_f)=Q_f x(t_f)$ 可知在终端时刻二者已是线性关系。
>
> - 假设为线性是出于对LQR结构的匹配式猜测(ansatz)：如果$\lambda$对$ x $是线性关系，那么协态方程保持先行，控制律保持线性，是闭合系统，否则很难闭合成可解的形式。
>
>   > Ansatz：德语，意思是“预设形式 / 试探假设 / 结构性猜测”。先根据问题结构“猜”解的形式，再代入方程验证是否自洽
>   >
>   > 本质：用“结构一致性 + 边界条件”指导解的形式，从而把问题闭合
>
> 然后我们也可以从HJB与PMP的一致性得到（可详看动态规划求解）
>
> - 记号区分：$J(x,t;u)$ 是给定控制 $u(\cdot)$ 的代价，$V(x,t)=\min_{u(\cdot)}J(x,t;u)$ 是最优值函数：
>   $$
>   V(x,t)=\min_{u(\cdot)}\left[h(x(t_f),t_f)+\int_t^{t_f} g(x(\tau),u(\tau),\tau)\,d\tau\right]
>   $$
> - LQR 为“线性系统 + 二次型代价”，最优值函数可写成二次型 $V(x,t)=x^T P(t) x$，因此
>   $$
>   \lambda(t)=\frac{\partial V}{\partial x}=2P(t)x(t)
>   $$
>   与状态线性相关。
> - PMP 的最小化条件与 HJB 的最小化条件逐项对应，因此
>   $$
>   \lambda(t)=\frac{\partial V}{\partial x}(x(t),t)
>   $$
>   是 PMP 与 DP 的一致性结果；在该结构下线性关系可闭合得到 Riccati 方程。
...
<img src="assets/04-连续最优控制.assets/image-20250612201343497.png" alt="image-20250612201343497" style="zoom:50%;" />
