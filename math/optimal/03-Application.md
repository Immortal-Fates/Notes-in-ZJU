...
1. 使用正则化逼近得到问题的稀疏解、解析解分别需要使用什么类型的范数？

  - 稀疏解：常用 $$l_1$$ 范数正则（如 LASSO），促使解出现零元素。
  - 解析（平滑/小能量）解：常用 $$l_2$$ 范数正则（如 Tikhonov / Ridge），得到闭式解且更平滑。

2. 最小二乘的解析解是什么？成立条件是什么？
...
3. $$l_\infty$$ 范数逼近如何改写成线性规划？
  ...
  - 等价 LP：
    $$
    \min t\quad \text{s.t.}\quad -t\le a_i^T x-b_i\le t,\ \forall i
    $$
...
4. $$l_1$$ 范数逼近如何改写成线性规划？
  ...
  - 等价 LP：
    $$
    \min \mathbf{1}^T t\quad \text{s.t.}\quad -t_i\le a_i^T x-b_i\le t_i,\ \forall i
    $$

5. LASSO 的目标形式与等价约束形式是什么？
  ...
  - 等价约束形式：
    $$
    \min_x \|Ax-b\|_2^2\quad \text{s.t.}\quad \|x\|_1\le c
    $$

6. Tikhonov 正则化（岭回归）的闭式解是什么？
...
7. 为什么 $$l_1$$ 正则比 $$l_2$$ 正则更容易得到稀疏解？

  - 几何直觉：$$l_1$$ 范数球有尖角，最优解更容易落在坐标轴上，从而产生零元素。

8. Huber 罚函数的分段形式与意义是什么？
  ...
  - 定义：
    ...
    \end{cases}
    $$
  - 意义：小误差平方惩罚，大误差线性惩罚，增强鲁棒性。

9. 最小范数问题在欠定方程中的意义是什么？

  - 当 $$Ax=b$$ 解不唯一时，通过
    $$
    \min \|x\|\quad \text{s.t.}\quad Ax=b
    $$
    选取“能量最小 / 最平滑 / 最稀疏”的代表解。

10. Square-root LASSO 的形式与常见用途是什么？

  - 形式：
    $$
    \min_x \|Ax-b\|_2+\gamma\|x\|_1
    $$
  - 用途：凸优化求解，得到稀疏解后可用于特征筛选。
