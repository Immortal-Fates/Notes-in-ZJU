...
- 光轴穿过图像平面的点称为图像中心
  ...
  ![image-20250610120604700](assets/数字图像处理——相机标定与三维重建.assets/image-20250610120604700.png)
...
- 线性摄像机成像模型
  ...
  <img src="assets/数字图像处理——相机标定与三维重建.assets/image-20250611000902412.png" alt="image-20250611000902412" style="zoom:50%;" />
  $$
...
- 内参矩阵
  ...
  K的常见形式为:$\begin{bmatrix} f_x & 0 & c_x \\ 0 & f_y & c_y \\ 0 & 0 & 1 \end{bmatrix}$

  各项含义：

    - $f_x, f_y$：focal length，用像素单位表示。它控制投影的缩放强度。值越大，物体投到图像上越大，类似“更长焦”。
    - $c_x, c_y$：principal point，也就是相机光轴穿过图像平面的位置，通常接近图像中心。
  
  对一个 camera coordinate 下的 3D 点 $(X,Y,Z)$，投影关系可以写成：
  ...
  $$
    ...
    - $\frac{X}{Z}, \frac{Y}{Z}$：透视投影，越远的点 $Z$ 越大，投影越靠近中心。
    - $f_x, f_y$：把归一化成像平面坐标放大成像素尺度。
    - $c_x, c_y$：把原点从相机光轴中心平移到图像像素坐标系。
...
- 相机畸变：
  ...
  - 径向畸变：沿着透镜半径方向分布的畸变
    ...
    ![image-20250620062254890](assets/数字图像处理——相机标定与三维重建.assets/image-20250620062254890.png)

  - 切向畸变：切向畸变是由于透镜本身与相机传感器平面（像平面）或图像平面不平行而产生的。

    ![image-20250620062336054](assets/数字图像处理——相机标定与三维重建.assets/image-20250620062336054.png)
...
- 相机标定方法
  ...
  - 径向畸变的相机标定
    ...
    ![image-20250611003748089](assets/数字图像处理——相机标定与三维重建.assets/image-20250611003748089.png)
