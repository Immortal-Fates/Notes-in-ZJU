...
其中 $f_i(\theta)$ 通常代表第 $i$ 个样本的损失（如交叉熵或平方误差）。
...
> Tips：你在一个起伏复杂的地形中“下山”。优化算法决定你如何选择方向和步幅。

## 基本记号与优化问题设定

- 参数：$\theta \in \mathbb{R}^d$
- 样本：$(x_i, y_i)$
- 损失函数：$f_i(\theta)$
- 经验风险：$f(\theta) = \frac{1}{n}\sum_i f_i(\theta)$
- 梯度：$\nabla f(\theta) = [\partial f/\partial \theta_1, \dots, \partial f/\partial \theta_d]^T$
- 学习率：$\eta$
...
> Tips: Adam 通常在早期收敛更快，但 SGD + Momentum 在后期泛化更好。
